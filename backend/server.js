...
// POST / students- RECIEVES new student data and inserts it into my SQL
app.post("/students", (req, res) => {
    ...
    db.query(sql, [first_name, last_name, grade_level], (error, results) => {
        if(error) {
            ...
            res.status(201).json({message: "student added succesfully", studentId: results.insertId});
        });
    });
...
app.post("/login", (req, res) => {
    ...
    db.query(sql, [email], (error, results) => {
        ...
        res.status(200)({
            message:"login successful", first_name: user.first_name, last_name: user.last_name, student_id: user.student_id
        });
    });
});
...
app.get("/classes",(req, res) => {
    ...
    db.query(sql,(error, results) => {
        if(error){
            console.error("eror getting classes:", error);
...
// GET /enrollments-returns joined data(student name plus class name)

app.get("enrollments", (req, res) => {
    const sql = "SELECT students.first_name, students.last_name, classes.class_name, classes.teacher_name FROM enrollments JOIN students ON enrollment.student_id = students.id JOIN classes ON enrollments.class_id = classes.id";
...
// GET /students/:id/grades-returns all grades for a specific student by id
